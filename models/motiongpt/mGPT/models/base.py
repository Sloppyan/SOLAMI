...
class BaseModel(LightningModule):
    ...
    def save_npy(self, outputs):
        ...
        if cfg.TEST.SAVE_PREDICTIONS:
            if 'TASK' in cfg.TRAIN and cfg.TRAIN.TASK == 'interaction':
                for batch_id in range(len(outputs)):
                    cur_bacth = outputs[batch_id]
                    for j in range(len(cur_bacth['length_inputs'])):
                        save_data = {}
                        save_data['joints_input'] = cur_bacth['joints_input'][j][:cur_bacth['length_inputs'][j]].cpu().numpy().tolist()
                        save_data['joints_ref'] = cur_bacth['joints_ref'][j][:cur_bacth['length'][j]].cpu().numpy().tolist()
                        save_data['joints_rst'] = cur_bacth['joints_rst'][j][:cur_bacth['length'][j]].cpu().numpy().tolist()
                        save_data['speech_input'] = cur_bacth['t_input'][j]
                        save_data['speech_output_ref'] = cur_bacth['t_ref'][j]
                        save_data['speech_output_rst'] = cur_bacth['t_rst'][j]
                        save_name = str(cur_bacth['csv_id'][j]) + '_results.json'
            ...
            if cfg.TRAIN.STAGE == 'vae':
                ...
                recon_joints = [i[0] for i in outputs]
                ref_joints = [i[2] for i in outputs]
                keyids = self.trainer.datamodule.test_dataset.name_list
                for i in range(len(ref_joints)):
                    for bid in range(min(cfg.TEST.BATCH_SIZE, ref_joints[i].shape[0])):
                        keyid = keyids[i * cfg.TEST.BATCH_SIZE + bid]
                        recon_joint = recon_joints[i][bid, :lengths[i][bid]].cpu().numpy()
                        ref_joint = ref_joints[i][bid, :lengths[i][bid]].cpu().numpy()
                        if cfg.TEST.REPLICATION_TIMES > 1:
                            name = f"{keyid}_{self.rep_i}.npy"
                        else:
                            name = f"{keyid}.npy"
                        npypath = output_dir / name
                        np.save(npypath, recon_joint)
                        npypath = output_dir / f"{keyid}_gt.npy"
                        np.save(npypath, ref_joint)
                return 
