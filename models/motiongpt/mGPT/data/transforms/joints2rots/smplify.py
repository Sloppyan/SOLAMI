...
class SMPLify3D():
    ...
    def __call__(self, init_pose, init_betas, init_cam_t, j3d, conf_3d=1.0, seq_ind=0):
        ...
        init_cam_t = init_cam_t.unsqueeze(1)
       ...
       # -------------Step 1: Optimize camera translation and body orientation--------
        ...
        if self.use_lbfgs:
            ...
            # for i in range(self.num_iters):
                def closure():
                    ...
                    loss.backward()
                    print(f"LBFGS iter: {i}, loss: {loss.item()}")
                    return loss
            ...
            for i in tqdm(range(self.num_iters), desc=f"Adam iter: "):
                smpl_output = self.smpl(global_orient=global_orient,
                                        body_pose=body_pose,
                                        betas=betas)
                model_joints = smpl_output.joints
                ...
                body_optimizer.zero_grad()
                loss.backward()
                print(f"Adam iter: {i}, loss: {loss.item()}")
                body_optimizer.step()
        ...
        return vertices, joints, pose, betas, camera_translation.squeeze(1), final_loss