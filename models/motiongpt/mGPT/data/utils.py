...
def humanml3d_collate(batch):
    ...
    if notnone_batches[0][0] is not None:
        adapted_batch.update({
            ...
            "all_captions": [b[3] for b in notnone_batches],
        })

    # Tasks
    if len(notnone_batches[0]) == 7:
        adapted_batch.update({"tasks": [b[6] for b in notnone_batches]})
    
    # if len(notnone_batches[0]) == 10:
    adapted_batch.update({
        'transform': collate_tensors([torch.tensor(b[4]).float() for b in notnone_batches]),
        'partner_motion': [torch.tensor(b[5]).float() if b[5] is not None else None for b in notnone_batches],
