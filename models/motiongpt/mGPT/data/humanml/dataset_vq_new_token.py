...
class Text2MotionVQDatasetToken(data.Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Fast loading
        if os.path.exists(pjoin(data_root, f'tmp_new/{split}{subset}_{motion_type}_{motion_part}_data.pkl')):
            if tiny or debug:
                with open(pjoin(data_root, f'tmp_new/{split}{subset}_{motion_type}_{motion_part}_data.pkl'),
                          'rb') as file:
                    data_dict = pickle.load(file)
            else:
                with rich.progress.open(
                        pjoin(data_root, f'tmp_new/{split}{subset}_{motion_type}_{motion_part}_data.pkl'),
                        'rb',
                        description=f"Loading HumanML3D {split}") as file:
                    data_dict = pickle.load(file)
            with open(pjoin(data_root, f'tmp_new/{split}{subset}_{motion_type}_{motion_part}_index.pkl'),
                      'rb') as file:
            ...
            for idx, name in enumerator:
                ...
                if self.cfg['EXPER']['transform'] == True:
                    ...
                    if 'ske_relative_cont6d' in transforms:
            ...
            if tmpFile:
                os.makedirs(pjoin(data_root, 'tmp_new'), exist_ok=True)
                with open(pjoin(data_root, f'tmp_new/{split}{subset}_{motion_type}_{motion_part}_data.pkl'),
                          'wb') as file:
    ...
    #     self.reset_max_len(self.max_length)
        print('Final loaded total %d dataset items' % len(self.name_list))
        self.pointer = 0
    ...
    def __getitem__(self, item):
        idx_item = self.pointer + item
        data_item = self.data_dict[self.name_list[idx_item]]
        ...
        return self.name_list[idx_item], motion, m_length, all_captions + [partner_name], transform, partner_motion
