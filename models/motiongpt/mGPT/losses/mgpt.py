...
class GPTLosses(BaseLosses):
    ...
    def __init__(self, cfg, stage, num_joints, **kwargs):
        ...
        if stage == "vae":
            ...
            losses.append("recons_feature_trans")
            params['recons_feature_trans'] = cfg.LOSS.LAMBDA_FEATURE

            losses.append("vq_commit_trans")
            params['vq_commit_trans'] = cfg.LOSS.LAMBDA_COMMIT
        ...
        if self.stage in ["vae"]:
            ...
            if self.cfg.EXPER.motion_part == 'body':
                if self.cfg.EXPER.motion_repre == 'ske':
                    velocity_index = [0,1,2, *range(4+21*9, 4+21*9+22*3)]
                else:
                    velocity_index = [*range(9), *range(9, 9+6*11)]
            else:
                if self.cfg.EXPER.motion_repre == 'ske':
                    velocity_index = [0,1,2, *range(4+51*9, 4+51*9+52*3)]
                else:
                    velocity_index = [*range(9), *range(9, 9+6*11)]
            
            total += self._update_loss(
                    "recons_velocity",
                    rs_set['m_rst'][..., velocity_index],
                    rs_set['m_ref'][..., velocity_index])        
            ...
            if 'loss_commit_trans' in rs_set:
                total += self._update_loss("vq_commit_trans", rs_set['loss_commit_trans'],
                                       rs_set["loss_commit_trans"])
            
            if 'm_trans_rst' in rs_set:
                total += self._update_loss("recons_feature_trans", rs_set["m_trans_rst"],
                                    rs_set["m_trans_ref"])
