...
class VQVAE_Trans(nn.Module):
    ...
    def __init__(self,
        ...
        self.nfeats = nfeats
        self.encoder = EncoderTrans(nfeats,
                               output_emb_width,)

        self.decoder = DecoderTrans(nfeats,
                               output_emb_width,)
    ...
    def forward(self, features: Tensor):
        ...
        x_encoder = x_encoder.view(B, -1, 1).contiguous()
        # quantization
        x_quantized, loss, perplexity = self.quantizer(x_encoder)
        x_quantized = x_quantized.view(B, -1).contiguous()
        # decoder
        x_decoder = self.decoder(x_quantized)
        x_out = self.postprocess(x_decoder)
    ...
    def decode(self, z: Tensor):
        ...
        x_d = self.quantizer.dequantize(z)
        x_d = x_d.view(-1, self.code_dim).contiguous()# .permute(0, 2, 1).contiguous()
...
class EncoderTrans(nn.Module):
    def __init__(self, input_dim=9, latent_dim=16):
        super().__init__()
        scale = 3
        self.fc1 = nn.Linear(input_dim, 16 * scale)
        self.fc2 = nn.Linear(16* scale, 32* scale)
        self.fc3 = nn.Linear(32* scale, latent_dim)
        self.relu = nn.ReLU()
...
class DecoderTrans(nn.Module):
    def __init__(self, output_dim=9, latent_dim=16):
        super().__init__()
        scale = 3
        self.fc1 = nn.Linear(latent_dim, 16* scale)
        self.fc2 = nn.Linear(16* scale, 32* scale)
        self.fc3 = nn.Linear(32* scale, output_dim)
        self.relu = nn.ReLU()
