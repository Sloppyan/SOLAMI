...
def calculate_rotvec_error(preds, target):
    ...
    ### get rotation matrix from rotation vector
    pred_rotmat = roma.rotvec_to_rotmat(preds)
    target_rotmat = roma.rotvec_to_rotmat(target)
    
    relative_rotmat = roma.rotmat_inverse(target_rotmat) @ pred_rotmat
    
    ### get relative rotation angle
    relative_rotvec = roma.rotmat_to_rotvec(relative_rotmat)
    relative_angle = torch.norm(relative_rotvec, dim=-1)
    relative_angle = torch.mean(relative_angle, dim=-1)
