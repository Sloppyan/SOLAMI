...
def preprocess_smplx(source_data, debug):
    # Uniformly scale the skeleton to match a target offset
    ...
    # HumanML3D w, xyz; roma xyz, w attention the order of quaternion!
...
if __name__ == "__main__":
    ...
    data_dir = 'SOLAMI_data/HumanML3D/HumanML3D_no_mirror'
    # change your save folder
    save_dir = 'SOLAMI_data/HumanML3D/unified_data/'

    dataset_items_info_path = "SOLAMI_data/HumanML3D/dataset_items_pre.json"
    ...
    for source_file in tqdm(source_list):
        try:
            data_item = dataset_items_info[source_file]
            motion_data_path = os.path.join(data_dir, data_item['motion_data_path'])
            if motion_data_path.endswith('npy'):
                continue
            source_data_ = np.load(motion_data_path, allow_pickle=True)
            start_frame = data_item['start_frame']
            end_frame = data_item['end_frame']
            source_data_ = dict(source_data_)
            source_data = {}
            source_data['smplx_params'] = {}
            for key in source_data_.keys():
                if key != 'pose_np':
                    if key != 'gender':
                        source_data['smplx_params'][key] = source_data_[key][start_frame:end_frame]
                    else:
                        source_data['smplx_params'][key] = source_data_[key]
                else:
                    source_data['ske_joints'] = source_data_[key][start_frame:end_frame]
            
            data, ground_positions, positions, l_velocity, (ske_forward, ske_root, scale, forward_quat) = process_file(source_data, 0.002)
            rec_ric_data = recover_from_ric(torch.from_numpy(data).unsqueeze(0).float(), joints_num)
            source_data['ske_joints'] = rec_ric_data.squeeze().numpy()
            source_data['ske_feature'] = data
            
            source_data_smplx_params, joints_smplx, (smplx_forward, smplx_root) = preprocess_smplx(copy.deepcopy(source_data['smplx_params']), args.debug)
            source_data['smplx_params'] = source_data_smplx_params
            smplx_feature = get_smplx_feature(copy.deepcopy(source_data['smplx_params']), args.debug)
            source_data['smplx_feature'] = smplx_feature

            data_transforms = {
                "ske_forward": ske_forward,
                "ske_root": ske_root,
                "scale": scale,
                "smplx_forward": smplx_forward,
            ...
            # os.makedirs(os.path.dirname(motion_data_path.replace('HumanML3D_mirror', 'unified_data')), exist_ok=True)
            # if args.debug:
            #     np.save(source_file.replace('HumanML3D_mirror', 'processed_data').replace('.npz', '_joints.npy'), rec_ric_data.squeeze().numpy())
            #     np.save(source_file.replace('HumanML3D_mirror', 'processed_data').replace('.npz', '_smplx.npy'), joints_smplx.squeeze())
                # np.save(source_file.replace('HumanML3D_mirror', 'processed_data').replace('.npz', '_smplx_new.npy'), joints_smplx_new.squeeze())
            
            np.savez(os.path.join(save_dir, data_item['motion_name'] + '.npz'), **source_data)
